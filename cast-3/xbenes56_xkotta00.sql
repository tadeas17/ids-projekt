...
select "pivovar"."nazev" as "Jméno pivovaru", "pivo"."nazev" as "Pivo"
from "pivovar"
         left join "pivo" on "pivovar"."id" = "pivo"."id_pivovaru"
order by "pivovar"."nazev";
/*Zobrazi vsechny uvarene varky*/
select p."nazev"                               as "Pivo",
       v."objem[l]"                            as "Objem [l]",
       TO_CHAR(v."datum_vareni", 'DD.MM.YYYY') as "Datum uvaření",
       v."forma_distribuce"                    as "Forma distribuce",
       v."cena"                                as "Cena"
from "varka" v
         join "pivo" p on p."id" = v."id_pivo"
order by p."nazev";
/*Zobrazi vsechny hodnoceni, pro vsechny hospody*/
select hos."nazev"                                  as "Název hospody",
       hod."hodnota"                                as "Počet hvězd",
       concat(u."jmeno", concat(' ', u."prijmeni")) as "Uživatel",
       hod."komentar"                               as "Komentář"
from "hospoda" hos
         left join "hodnoceni_hospody" hod on hos."id" = hod."id_hospody"
         join "uzivatel" u on hod."login_uzivatel" = u."login"
order by hos."nazev";
/*Zobrazi kolik objemu piva celkem vypil každý uživatel, pokud nějáké vypil*/
select concat(u."jmeno", concat(' ', u."prijmeni")) as "Uživatel",
       sum(svp."objem_vypiteho_piva[ml]")           as "Objem vypitého piva [ml]"
from "uzivatel" u
         join "seznam_vypitych_piv" svp on u."login" = svp."login_uzivatel"
group by u."login", u."jmeno", u."prijmeni"
order by concat(u."jmeno", concat(' ', u."prijmeni"));
/*Zobrazí počet druhů piv od každého pivovaru*/
select pr."nazev" as "Pivovar", count(pv."id") as "Počet druhů piv"
from "pivovar" pr
         left join "pivo" pv on pr."id" = pv."id_pivovaru"
group by pr."nazev"
order by pr."nazev";
...
where exists(select 1
             from "hodnoceni_hospody" hod
             where hod."id_hospody" = hos."id"
             group by hod."id_hospody"
             having avg(hod."hodnota") >= 3.5);
/*Zobrazí všechna piva s hodnocením větším nebo rovno 3.5*/
select pv."nazev" as "Pivo"
from "pivo" pv
where "id" IN (select hod."id_pivo" from "hodnoceni_piva" hod group by hod."id_pivo" having avg(hod."hodnota") >= 3.5);
...
begin
    hodnoceni_hospody_procedure('xkott00', 1, 5, 'Super');
end;
...
grant all on "uzivatel" to xbenes56;
...
begin
    dbms_output.put_line('TEST POPISU');
end;